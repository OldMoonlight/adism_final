<?php require_once(__DIR__."/../includes/cURL_reCaptcha.php");
      require_once(__DIR__."/../includes/csrf.class.php");
      require_once(__DIR__."/../includes/SQLiteLogins.php");
/**
...
{	
    ...
    public function runApplication()
    {
        ...
        if (isset($_GET["action"]) && $_GET["action"] == "register" && isset($_SESSION['user_name']) && !empty($_SESSION['user_name'])) {
            $this->doRegistration();
            ...
            if ($this->getUserLoginStatus()) {
				...
				//can create new user by replacing doRegistration() here
            }
        }
    }
    ...
    private function checkPasswordCorrectnessAndLogin()
    {
        ...
        if(isset($_POST['csrf_token']) && !empty($_POST['csrf_token'])){
            $csrf = new csrf;
            if($csrf::isTokenValid() === false){
                $this->feedback = "CSRF mismatch. Please try again.";
                return false;
            }
        }
        ...
        if(isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])){
            $result = reCAPTCHA_Verify($_POST['g-recaptcha-response']);
            if ($result != true){
                $this->feedback = "reCaptcha failed. Contact the admin";
                var_dump($result);
                return false;
            }
        } else {
            // comment this for testing AND REMEMBER TO REMOVE IT or find a better way to test...
            $this->feedback = "Please check reCaptcha.";
            return false;
        }
        */
        ...
        if ( $result_row ){ //TO FIX: if ( $result_row ? $this->maxLoginsNotExceeded($result_row) : false ){
            ...
            if (password_verify($_POST['user_password'], $result_row->user_password_hash)) {
				...
				/*$this->db_connection->prepare("UPDATE users SET failed_logins = failed_logins + 1")->execute();
				$this->db_connection->prepare("UPDATE users SET last_failed = ".mktime())->execute();*/
            }
        } else {
	...
	/*
	private function maxLoginsNotExceeded($result_row){
		date_default_timezone_set('America/Port_of_Spain');
		$today = date("y-m-d", time());
		$date_last_failed = date("y-m-d", $result_row->last_failed);
		if ( $date_last_failed =! $today ){
			$sql = 'UPDATE users
					SET failed_logins = 0
					WHERE user_name = :user_name OR user_email = :user_name
					LIMIT 1';
		...
		if ( $result_row->failed_logins >= $this->MAX_PERIODIC_FAILS ){
			return false;
		}
		return true;
	}*/
    ...
    {
		include_once(__DIR__."/../auth/index.html");

        if ($this->feedback) {
            include_once(__DIR__."/../templates/bottomSheetRender.php");
            $sheet = new bottomSheet;
            $sheet::Render($this->feedback, "" ,"OK");
        }
